...
class SearchWorker(QThread):
    ...
    log = pyqtSignal(str)
    source_result = pyqtSignal(str, list)
    ...
    def run(self):
        ...
        try:
            with ProxyEnv(self.proxy_enabled, self.proxy_host, self.proxy_port):
                ...
                self.log.emit(f"开始并发搜索，共 {total} 个音源，结果将逐步显示。")
                    ...
                    if isinstance(result, dict):
                        ...
                        for value in result.values():
                            if isinstance(value, list):
                                songs.extend(value)
                        result = songs
                    return source, result if isinstance(result, list) else []

                max_workers = min(max(total, 1), 12)
                executor = ThreadPoolExecutor(max_workers=max_workers, thread_name_prefix="music-search")
                pending = {}
                try:
                    pending = {executor.submit(search_one, src): src for src in self.selected_sources}
                    while pending:
                        if self.cancel_event.is_set():
                            self.log.emit("正在取消搜索，停止等待未完成音源...")
                        ...
                        for future in completed:
                            ...
                            try:
                                ...
                                merged[source] = songs
                                self.source_result.emit(source, songs)
                                self.log.emit(f"{source} 搜索完成，结果 {len(songs)} 首")
...
class DownloadWorker(QThread):
    ...
    def __init__(self, songs, limit, save_dir, proxy_enabled, proxy_host, proxy_port):
        super().__init__()
        self.songs = songs
        self.limit = limit
...
class ImageSignals(QObject):
    finished = pyqtSignal(int, int, str, QPixmap)
    error = pyqtSignal(int, int)


class ImageTask(QRunnable):
    def __init__(self, row, image_url, signals, generation):
        super().__init__()
        self.row = row
    ...
    def run(self):
        try:
            if not self.image_url:
                self.signals.error.emit(self.row, self.generation)
                return
            ...
            self.signals.finished.emit(self.row, self.generation, self.image_url, scaled)
        except Exception:
            self.signals.error.emit(self.row, self.generation)
...
class MusicDownloader(QMainWindow):
    def __init__(self):
        ...
        self.download_dir = os.path.join(self.current_dir, "已下载音乐")
        ...
        self.image_pool.setMaxThreadCount(12)
    ...
    def setup_controls(self, parent_layout):
        ...
        self.btn_browse_download_dir = QPushButton("选择下载目录")
        self.btn_browse_download_dir.clicked.connect(self.on_browse_download_dir)
        ...
        row1.addWidget(self.download_dir_edit, 1)
        row1.addWidget(self.btn_browse_download_dir)
        row1.addSpacing(16)
        row1.addWidget(self.check_auto_download)
        top_layout.addLayout(row1)

        row_cache = QHBoxLayout()
        self.search_cache_dir_edit = QLineEdit(self.search_cache_dir)
        self.search_cache_dir_edit.setReadOnly(True)
        self.btn_browse_cache_dir = QPushButton("选择缓存目录")
        self.btn_browse_cache_dir.clicked.connect(self.on_browse_cache_dir)
        row_cache.addWidget(QLabel("搜索缓存目录"))
        row_cache.addWidget(self.search_cache_dir_edit, 1)
        row_cache.addWidget(self.btn_browse_cache_dir)
        top_layout.addLayout(row_cache)
    ...
    def on_browse_download_dir(self):
        target = QFileDialog.getExistingDirectory(self, "选择下载目录", self.current_dir)
        if target:
            self.download_dir = target
            self.download_dir_edit.setText(target)
            self.append_log(f"下载目录已设置为：{target}")

    def on_browse_cache_dir(self):
        target = QFileDialog.getExistingDirectory(self, "选择搜索缓存目录", self.current_dir)
        if target:
            self.search_cache_dir = target
            self.search_cache_dir_edit.setText(target)
            self.append_log(f"搜索缓存目录已设置为：{target}")
    ...
    def on_search(self):
        ...
        self.search_worker.source_result.connect(self.on_search_source_result)
    ...
    def on_search_source_result(self, source, songs):
        self.search_results[source] = songs
        if songs:
            self.append_songs_to_table(songs)
            total_count = sum(len(items) for items in self.search_results.values())
            source_cn = self.source_map_en_to_cn.get(source, source)
            self.status_label.setText(f"{source_cn} 已返回，当前共 {total_count} 首")
            self.btn_download.setEnabled(total_count > 0)
    ...
    def on_search_finished(self, search_results):
        ...
        if self.search_mode.currentText() == "解析歌单链接":
            self.music_records = {}
            self.clear_results_table()
            self.append_songs_to_table(all_songs)
    ...
    def append_songs_to_table(self, all_songs):
        start_row = self.results_table.rowCount()
        self.results_table.setRowCount(start_row + len(all_songs))
        generation = self.results_generation
        for offset, song in enumerate(all_songs):
            ...
            if cover_url in self.image_cache:
                self.on_image_downloaded(row, generation, cover_url, self.image_cache[cover_url])
            else:
                self.results_table.setCellWidget(row, 1, self.build_cover_placeholder())
                task = ImageTask(row=row, image_url=cover_url, signals=self.image_signals, generation=generation)
                self.image_pool.start(task)
        self.btn_download.setEnabled(len(all_songs) > 0)
    ...
    def on_image_downloaded(self, row, generation, image_url, pixmap):
        if generation != self.results_generation or row >= self.results_table.rowCount():
            return
    ...
    def on_image_error(self, row, generation):
        if generation == self.results_generation and row < self.results_table.rowCount():
            self.results_table.setCellWidget(row, 1, self.build_cover_placeholder())
